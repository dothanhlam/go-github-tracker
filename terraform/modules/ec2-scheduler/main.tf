# IAM Role for EventBridge Scheduler to call EC2 APIs
resource "aws_iam_role" "ec2_scheduler" {
  name = "${var.project_name}-${var.environment}-ec2-scheduler-role"

  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect = "Allow"
      Principal = {
        Service = "scheduler.amazonaws.com"
      }
      Action = "sts:AssumeRole"
    }]
  })

  tags = {
    Name = "${var.project_name}-${var.environment}-ec2-scheduler-role"
  }
}

resource "aws_iam_role_policy" "ec2_scheduler" {
  name = "${var.project_name}-${var.environment}-ec2-scheduler-policy"
  role = aws_iam_role.ec2_scheduler.id

  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Effect = "Allow"
        Action = [
          "ec2:StopInstances",
          "ec2:StartInstances"
        ]
        Resource = var.instance_arn
      }
    ]
  })
}

# Scheduler Group
resource "aws_scheduler_schedule_group" "ec2" {
  name = "${var.project_name}-${var.environment}-ec2-scheduler"

  tags = {
    Name = "${var.project_name}-${var.environment}-ec2-scheduler"
  }
}

# Stop EC2 at 10:00 PM (Asia/Ho_Chi_Minh)
resource "aws_scheduler_schedule" "ec2_stop" {
  name       = "${var.project_name}-${var.environment}-ec2-stop"
  group_name = aws_scheduler_schedule_group.ec2.name

  flexible_time_window {
    mode = "OFF"
  }

  schedule_expression          = var.stop_schedule
  schedule_expression_timezone = var.timezone

  target {
    arn      = "arn:aws:scheduler:::aws-sdk:ec2:stopInstances"
    role_arn = aws_iam_role.ec2_scheduler.arn

    input = jsonencode({
      InstanceIds = [var.instance_id]
    })

    retry_policy {
      maximum_retry_attempts = 3
    }
  }
}

# Start EC2 at 10:00 AM (Asia/Ho_Chi_Minh)
resource "aws_scheduler_schedule" "ec2_start" {
  name       = "${var.project_name}-${var.environment}-ec2-start"
  group_name = aws_scheduler_schedule_group.ec2.name

  flexible_time_window {
    mode = "OFF"
  }

  schedule_expression          = var.start_schedule
  schedule_expression_timezone = var.timezone

  target {
    arn      = "arn:aws:scheduler:::aws-sdk:ec2:startInstances"
    role_arn = aws_iam_role.ec2_scheduler.arn

    input = jsonencode({
      InstanceIds = [var.instance_id]
    })

    retry_policy {
      maximum_retry_attempts = 3
    }
  }
}
